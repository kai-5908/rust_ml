...
pub fn validate_positive_float_from_series(float_series: &Series) {
    let series_iter = float_series.f64().expect("series was not an f64 dtype").into_iter();
...
pub fn validate_sex_from_series(str_series: &Series) {
    let series_iter = str_series.utf8().expect("series was not an utf8 dtype").into_iter();
    for val in series_iter{
        match val{
